...
*/
const double=[1,2,3];
double.forEach(function doubleValues(num, index, arr){
 arr[index]= num *2;
});
...
const even=[1,2,3];
const newEven=[];
even.forEach(function onlyEvenValues(evenNum){
  if(evenNum % 2 === 0){
    newEven.push(evenNum);
  }
});
console.log(newEven);    
...
*
*/
const stringsNames=['colt','matt', 'tim', 'udemy'];
const flchar =[];

stringsNames.forEach(function showFirstAndLast(name){
  flchar.push(name.charAt(0)+ name.charAt(name.length-1));
});
...
addKeyAndValue(obj, 'title', 'instructor');
 console.log(obj);
