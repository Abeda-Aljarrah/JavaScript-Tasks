...
const avgAge= persons.reduce(function(acc,currently) {
  return acc + currently.age;},0)
  console.log(avgAge/persons.length);
...
*/
  ...
  const longestName =persons.reduce(function(longest, currentlyPerson) {
    const fullName = `${currentlyPerson.name.first} ${currentlyPerson.name.last}`;
  ...
  /**
  *!we should but an initial value " ", if we don't use it, it will compare object with string which is undefined  */
...
arr=[1,2,4,9];
const maxNumber = arr.reduce(function(acc,currently) {
    return Math.max(acc,currently);
},);
...
// word ="hello world"
// const repeatChar = word.filter(function(
...
*/
even=[1,8,6,4]
const evenOnly = even.filter(function (num){
  return num % 2 === 0;
});
...
*/
multiFour=[1,8,6,4]
const num = multiFour.filter(function (multi){
  return multi % 4 === 0;
});
console.log(num);
...
var strings= ["alex","mercer","madrasa","rashed2","emad","hala"]
const evenIndexOddLength = strings.filter(function (string,index){
  if (string.length % 2 === 1 && index % 2 ===0) {
    return string;
  }
});
